...
begin
  ...
  -- Use one lock order for all participant deal mutations: deal first, then
  -- the caller's exact membership. A concurrent self-block either finishes
  -- first and makes this predicate fail, or waits until this RPC commits.
  perform 1
...
begin
  ...
  -- The deal row is the first concurrency lock. A second participant racing
  -- the first observes the disputed state and receives the same live report id.
  select * into deal_record
  from public.deals d
  where d.id = target_deal_id
  for update;
