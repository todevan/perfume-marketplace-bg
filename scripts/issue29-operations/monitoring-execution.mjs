...
const database=z.object({scope:z.object({mode:z.literal('hosted'),role:z.enum(['source','target']),runId:z.uuid(),projectRef:z.string(),sourceRef:z.string(),preservedRefs:z.array(z.string()),createdResourceEvidenceSha256:hash,apiUrl:z.url()}).strict(),
  connection:z.object({host:z.string(),port:z.number().int(),user:z.string(),database:z.string(),password:secret,sslmode:z.literal('verify-full'),sslRootCert:z.enum(['system','supabase-prod-2021']).optional()}).strict(),
...
const resend=z.object({apiKey:secret,senderMode:z.literal('resend-account-test'),webhookId:z.uuid(),from:z.literal('onboarding@resend.dev'),to:z.email().refine(value=>!value.toLowerCase().endsWith('@resend.dev')),operationId:z.uuid(),windowStart:z.iso.datetime(),webhookOrigin:z.url(),syntheticScopeEvidenceSha256:hash,requireLiveQuota:z.boolean().optional(),
