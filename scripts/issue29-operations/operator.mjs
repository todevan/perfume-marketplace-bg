...
export async function executeMaintenanceLifecycle(options){
 ...
 else{ensure(['transient_cleanup_pending','incident_drill_verified','source_paused','source_resume_pending'].includes(manifest.state)&&['paused','resume_pending'].includes(maintenance.phase),'STATE_TRANSITION_FORBIDDEN');ensure((manifest.target?manifest.cleanup.resources.some(r=>r.provider==='supabase'&&r.id===manifest.target?.ref&&r.absentAt!==null):!manifest.attempts[maintenanceAttemptKey(manifest,'create-target')])&&manifest.cleanup.resources.filter(r=>r.disposition==='disposable'&&r.provider!=='grafana').every(r=>r.absentAt!==null),'RESTORE_TARGET_ABSENCE_REQUIRED');}
 if(!manifest.pending){
