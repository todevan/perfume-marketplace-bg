...
const receiptSchema = object({
  ...
  monitor: object({
    provider: z.literal('cloudflare-scheduled-free'), workerAlias: alias,
  ...
  alerts: object({
    deliveryEventType:z.literal('email.delivered'),
  ...
  cleanup: object({
    ...
    resources: z.array(object({
      provider: z.enum(['supabase', 'cloudflare', 'cloudflare-monitor']), resourceIdSha256: sha256,
    ...
    retainedResources: z.array(object({
      alias, kind: z.enum(['monitor-worker', 'resend-destination', 'monitor-rules', 'encrypted-backup', 'secondary-encrypted-copy', 'synthetic-source', 'synthetic-worker']), resourceIdSha256: sha256.optional()
    })).min(5).max(100),
...
export function validateOperationsReadiness(input, options = {}) {
  ...
  require(['monitor-worker', 'resend-destination', 'monitor-rules', 'encrypted-backup', 'secondary-encrypted-copy'].every((kind) => cleanup.retainedResources.some((resource) => resource.kind === kind)), 'retained resource inventory is incomplete');
  ...
  if(receipt.releaseUpdate){try{
    ...
    const transitionSchema=object({schemaVersion:z.literal(1),kind:z.literal('issue29-monitor-release-update'),evidenceMode:z.literal('provider-readback'),runId:identity,previousCandidateSha:gitSha,candidateSha:gitSha,previousConfigSha256:sha256,configSha256:sha256,environmentAlias:alias,origin:z.url(),verifiedAt:utc,protectedMergeEvidenceSha256:sha256,resource:object({provider:z.literal('cloudflare-monitor'),resourceId:identity,previousCandidateSha:gitSha,candidateSha:gitSha,priorStateSha256:sha256}),configuration:z.object({status:z.literal('verified'),candidateSha:gitSha,configSha256:sha256,workerAlias:alias}).passthrough()});
    ...
    require(transition.configuration.candidateSha===expected.commitSha&&transition.configuration.configSha256===monitor.configSha256&&transition.configuration.workerAlias===monitor.workerAlias,'merged monitor resource inventory does not match');
    require(transition.resource.previousCandidateSha===merge.fromCandidate.sha&&transition.resource.candidateSha===expected.commitSha&&transition.resource.resourceId===`worker:${monitor.workerAlias}`,'merged monitor exact resource readback does not match');
...
function assembleObserved(input){
 ...
 assert(monitor.status==='verified'&&monitor.configuration?.status==='verified'&&monitor.configuration.candidateSha===m.candidate.sha&&monitor.configuration.configSha256===m.monitoring.configSha256&&monitor.configuration.workerAlias===m.monitoring.workerAlias&&monitor.checks?.length===9&&new Set(monitor.checks.map((/** @type {any} */c)=>c.signal)).size===9&&monitor.checks.every((/** @type {any} */c)=>signals.includes(c.signal)&&c.ok===true)&&monitor.heartbeatAt===monitor.heartbeat?.heartbeatAt);
 assert(alerts.status==='verified'&&alerts.evidenceMode==='provider-readback'&&alerts.runId===m.runId&&alerts.candidateSha===release.sha&&alerts.destinationAlias==='owner-primary'&&alerts.timelines?.length===9&&alerts.ruleMappings?.length===9&&new Set(alerts.ruleMappings.map((/** @type {any} */r)=>r.ruleKey)).size===9&&signals.every(s=>alerts.ruleMappings.some((/** @type {any} */r)=>r.signal===s)));
 assert(alerts.sourceConfigSha256===(m.releaseUpdate?m.maintenance.monitoring.sourceConfigSha256:m.monitoring.configSha256));
 ...
 const retained=[{alias:'synthetic-source',kind:'synthetic-source',resourceIdSha256:hashId(m.source.ref)},{alias:'synthetic-worker',kind:'synthetic-worker',resourceIdSha256:hashId(worker.workerName)},{alias:m.monitoring.workerAlias,kind:'monitor-worker'},{alias:'owner-primary',kind:'resend-destination'},{alias:'launch-readiness',kind:'monitor-rules'},{alias:latest.metadata.destinationAlias,kind:'encrypted-backup',resourceIdSha256:hashId(String(artifact.artifactId))},{alias:copy.destinationAlias,kind:'secondary-encrypted-copy',resourceIdSha256:hashId(`owner-copy:${copy.descriptorSha256}`)}];
 const credentialCount=cleanupResources.filter(r=>r.provider==='supabase'||r.provider==='cloudflare').length+cleanup.observations.filter((/** @type {any} */o)=>o.provider==='cloudflare-monitor'&&o.resourceId.startsWith('secret:')).length+1;
