...
export async function runCli(args) {
  ...
  if(command==='deploy-monitor'){
    const values=argumentsFor(rest,['manifest','settings']);const manifest=await readPrivateManifest(values.manifest,{repositoryRoot});attestCandidate(manifest.candidate);
    const value=JSON.parse((await privateBytes(values.settings)).toString());ensure(value?.schemaVersion===1&&value.operation==='deploy-monitor'&&Object.keys(value).every(key=>['schemaVersion','operation','settings'].includes(key))&&value.settings,'PRIVATE_SETTINGS_INVALID');
    const {executeDeployMonitor}=await import('./monitor-deployment.mjs');
    return JSON.stringify(await executeDeployMonitor({manifestPath:values.manifest,repositoryRoot,candidate:manifest.candidate,settings:value.settings}))+'\n';
  }
