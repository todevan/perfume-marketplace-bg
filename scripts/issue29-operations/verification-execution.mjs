...
import {createMonitorAdapter,monitorConfigSchema,MONITOR_SIGNALS} from './monitor-adapter.mjs';
...
const finalSchema=z.strictObject({schemaVersion:z.literal(1),operation:z.literal('cleanup'),action:z.literal('finalize'),providerToken:secret,capabilityId:id,sourceMonitoringSettingsPath:z.string(),sourceWorker:workerRead,
 ...
 monitorWorkers:z.array(z.strictObject({workerAlias:id,accountId:z.string().regex(/^[a-f0-9]{32}$/u),readToken:secret})).max(10)});
/** @typedef {{fetchImpl?:typeof fetch,supabaseFactory?:typeof createSupabaseOperationsAdapter,workerFactory?:typeof createIssue29WorkerAdapter,sourceBinding?:typeof readSourceReleaseBinding,workerBinding?:typeof readIssue29WorkerBinding,monitorFactory?:typeof createMonitorAdapter}} CleanupDependencies */
...
export async function executeFinalizeCleanup(options,dependencies={}){
 ...
 return transaction(options,async(m,save,clock)=>{
  ...
  const monitorCapabilities=[ms.monitor.evidenceReadToken,ms.monitor.watchdogToken,ms.monitor.backupCheckpointToken,ms.monitor.maintenanceToken];
  ensure(!privateCapabilities.some(value=>monitorCapabilities.includes(value)),'MONITOR_CROSS_PROVIDER_CREDENTIAL_FORBIDDEN');
  ...
  for(const w of s.monitorWorkers){ensure(w.accountId===s.sourceWorker.settings.accountId&&w.workerAlias!==m.monitoring.workerAlias&&!mapping.has(`worker:${w.workerAlias}`),'FINAL_CLEANUP_MAPPING_INVALID');mapping.set(`worker:${w.workerAlias}`,'cloudflare-monitor');}
  const providerMapped=disposable.filter(r=>['cloudflare','cloudflare-monitor'].includes(r.provider));ensure(mapping.size===providerMapped.length&&providerMapped.every(r=>mapping.get(r.id)===r.provider),'FINAL_CLEANUP_MAPPING_INCOMPLETE');ensure(disposable.every(r=>['supabase','supabase-storage','cloudflare','cloudflare-monitor'].includes(r.provider)),'FINAL_CLEANUP_PROVIDER_UNSUPPORTED');
  ...
  for(const w of s.monitorWorkers){
   const response=await(dependencies.fetchImpl??fetch)(`https://api.cloudflare.com/client/v4/accounts/${w.accountId}/workers/scripts/${encodeURIComponent(w.workerAlias)}`,{headers:{Authorization:`Bearer ${w.readToken}`},redirect:'error',signal:AbortSignal.timeout(15000)});
   await response.body?.cancel();ensure(response.status===404,'FINAL_MONITOR_ABSENCE_UNPROVEN');observations.push({provider:'cloudflare-monitor',resourceId:`worker:${w.workerAlias}`,absent:true,checkedAt:clock()});
  }
  const g=(dependencies.monitorFactory??createMonitorAdapter)(ms.monitor,{fetchImpl:dependencies.fetchImpl,now:clock}),config=await g.verifyConfiguration();
  ensure((ms.monitor.targetRole??'source')==='source'&&config.runId===m.runId&&config.candidateSha===m.candidate.sha&&config.configSha256===m.monitoring.configSha256&&config.workerAlias===m.monitoring.workerAlias&&config.destinationAlias==='owner-primary'&&config.targetOrigin===ms.binding.deployment.origin&&ms.binding.source.apiUrl===source.url&&s.sourceWorker.settings.purpose==='source','FINAL_PERSISTENT_MONITOR_IDENTITY_MISMATCH');
  ...
  ensure(m.cleanup.resources.some(owned=>owned.provider==='cloudflare-monitor'&&owned.id===`worker:${config.workerAlias}`&&owned.disposition==='persistent'&&owned.absentAt===null),'PERSISTENT_MONITOR_OWNERSHIP_INCOMPLETE');
  const deferBackupFreshness=await hasCurrentPremergeOwnerCopy(m,options.manifestPath,options.repositoryRoot,clock());
  const current=await g.readSourceGreenState(),rules=current.signals;
  ensure(rules.length===MONITOR_SIGNALS.length&&new Set(rules.map(r=>r.signal)).size===MONITOR_SIGNALS.length&&rules.every(r=>MONITOR_SIGNALS.includes(r.signal)&&((deferBackupFreshness&&r.signal==='backup_freshness')||r.ok===true)),'PERSISTENT_MONITOR_NOT_GREEN');
  ensure(current.lastCompletedMonitorCycleAt&&Date.parse(clock())-Date.parse(current.lastCompletedMonitorCycleAt)>=0&&Date.parse(clock())-Date.parse(current.lastCompletedMonitorCycleAt)<=20*60*1000,'PERSISTENT_MONITOR_STALE');
