...
/** @typedef {{schemaVersion:1,policy:'issue29-owner-authorized-pending-source-readback',runId:string,operationId:string,organizationId:string,projectRef:string,region:string,sourceName:string,observedCreatedAt:string,authorizedAt:string,expiresAt:string,evidenceSha256:string}} OwnerSourceAuthorization */
...
export function validateOwnerSourceAuthorization(input, expected) { const v = /** @type {OwnerSourceAuthorization} */ (input); const keys = ['schemaVersion','policy','runId','operationId','organizationId','projectRef','region','sourceName','observedCreatedAt','authorizedAt','expiresAt','evidenceSha256']; ensure(v && Object.keys(v).length === keys.length && Object.keys(v).every(key => keys.includes(key)) && v.schemaVersion === 1 && v.policy === 'issue29-owner-authorized-pending-source-readback' && /^[a-f0-9]{64}$/u.test(v.evidenceSha256), 'OWNER_SOURCE_AUTHORIZATION_REQUIRED'); const m = expected.manifest; ensure(expected.purpose === 'source' && m.state === 'source_creation_pending' && m.pending?.step === 'create-source' && m.pending.operationId === expected.operationId && m.source === null && m.target === null && !m.preservedRefs.includes(expected.sourceRef) && !m.forbiddenRefs.includes(expected.sourceRef) && !m.cleanup.resources.some(r => r.provider === 'supabase' && r.id === expected.sourceRef && r.runId === m.runId), 'OWNER_SOURCE_AUTHORIZATION_SCOPE_FORBIDDEN'); ensure(v.runId === m.runId && v.operationId === expected.operationId && v.organizationId === m.provisioning.organizationId && v.projectRef === expected.sourceRef && v.region === m.provisioning.region && v.sourceName === expected.sourceName && v.observedCreatedAt === expected.observedCreatedAt, 'OWNER_SOURCE_AUTHORIZATION_MISMATCH'); ensure(Number.isFinite(Date.parse(expected.observedCreatedAt)) && Date.parse(expected.observedCreatedAt) <= Date.parse(expected.now) + 300000, 'CREATION_TIME_MISMATCH'); ensure(Number.isFinite(Date.parse(v.authorizedAt)) && Number.isFinite(Date.parse(v.expiresAt)) && Date.parse(v.authorizedAt) <= Date.parse(expected.now) && Date.parse(expected.now) < Date.parse(v.expiresAt), 'OWNER_SOURCE_AUTHORIZATION_EXPIRED'); return v; }
