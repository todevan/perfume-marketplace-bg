 ...
 * @param {{token:string,fetch?:typeof fetch,clock?:()=>string,readCapacityQuote?:(organizationId:string)=>Promise<CapacityQuote>,databasePassword?:(purpose:'source'|'target')=>Promise<string>,twoSlotAuthorization?:TwoSlotAuthorization,inspectEmpty?:(project:import('./manifest.mjs').ProjectIdentity)=>Promise<boolean>}} options
 */
export function createSupabaseOperationsAdapter(options) {
    ...
    async function readCreated(context) {
        ...
        const evidence={schemaVersion:1,kind:'issue29-supabase-created',runId:manifest.runId,operationId:context.operationId,project,createdAt:new Date(data.created_at).toISOString(),empty:empty===true,observed:{name:data.name,status:data.status,checkedAt:clock()}};return { project, createdAt:evidence.createdAt, foreignState: empty !== true, evidence,evidenceSha256: digest(evidence) };
    }
    /** @param {Context} context */
