 ...
 */
export async function readSourceReleaseBinding(options) { const proof = await readProjectReleaseBinding(options, 'source'); ensure('sourceRef' in proof, 'SOURCE_BINDING_IDENTITY_MISMATCH'); return proof; }
/** Exact target binding never depends on the paused source health. @param {Parameters<typeof readSourceReleaseBinding>[0]} options */
export async function readTargetReleaseBinding(options) { const proof = await readProjectReleaseBinding(options, 'target'); ensure('targetRef' in proof, 'SOURCE_BINDING_IDENTITY_MISMATCH'); return proof; }
/** @param {{manifest:import('./manifest.mjs').OperationsManifest,settings:BindingSettings,fetchImpl?:typeof fetch,now?:string}} options @param {'source'|'target'} role */
async function readProjectReleaseBinding({ manifest, settings, fetchImpl = fetch, now = new Date().toISOString() }, role) {
    const source = role === 'source' ? manifest.source : manifest.target;
    const expectedVersion = role === 'source' ? manifest.candidate.deploymentId : manifest.targetDeploymentId;
    const expectedName = role === 'source' ? `issue29-${manifest.runId}` : `issue29-restore-${manifest.maintenance?.id ?? manifest.runId}`;
    ensure(source && !manifest.preservedRefs.includes(source.ref), 'PRESERVED_PROJECT_FORBIDDEN');
    const { deployment } = settings;
    ensure(source.url === settings.source.apiUrl && /^[a-f0-9]{32}$/u.test(deployment.accountId) && /^issue29-[a-z0-9-]{1,55}$/u.test(deployment.workerName) && deployment.workerName === expectedName && deployment.versionId === expectedVersion && /^[a-zA-Z0-9-]{1,128}$/u.test(deployment.versionId), 'SOURCE_BINDING_IDENTITY_MISMATCH');
    ...
    for (const value of [settings.providerToken, settings.source.serviceKey, deployment.readToken])
        ...
        try {
            ...
            try {
                for (;;) {
                    ...
                    ensure(size <= 2097152, 'SOURCE_BINDING_RESPONSE_LIMIT');
            ...
            if (error instanceof OperationsError)
                throw error;
            throw new OperationsError('SOURCE_BINDING_READ_FAILED');
        }
    }
    ...
    const fingerprint = /** @param {string} value */ /** @param {string} value */ value => createHash('sha256').update(value).digest();
    ...
    const exact = (name, value) => bindings.filter(/** @param {any} b */ /** @param {any} b */ b => b.type === 'plain_text' && b.name === name && b.text === value).length === 1;
    ...
    ensure(Number.isFinite(Date.parse(version.metadata?.created_on)) && Date.parse(version.metadata.created_on) >= Date.parse(role === 'source' ? (manifest.sourceProvenance?.createdAt ?? '') : (manifest.cleanup.resources.find(r => r.provider === 'supabase' && r.id === source.ref && r.runId === manifest.runId && r.disposition === 'disposable' && r.absentAt === null)?.createdAt ?? '')) && Date.parse(version.metadata.created_on) <= Date.parse(now), 'WORKER_TRANSACTION_WINDOW_MISMATCH');
    ...
    try {
        const health = await fetchImpl(`${origin.origin}/`, { method: 'GET', redirect: 'error', signal: AbortSignal.timeout(15000) });
        await health.body?.cancel();
        ensure(health.status === 200 && health.headers.get('x-deployed-git-sha') === manifest.candidate.sha, 'WORKER_HEALTH_IDENTITY_MISMATCH');
    }
    catch (error) {
        if (error instanceof OperationsError)
            throw error;
    ...
    const proof = { ...(role === 'source' ? { sourceRef: source.ref } : { targetRef: source.ref }), organizationId: source.organizationId, region: source.region, candidate: manifest.candidate, workerName: deployment.workerName, runId: manifest.runId, checkedAt: now };
