...
The repair candidate has one deliberately narrow recovery-adoption capability. It may clean the failed `a9d55c0ef1138dfb33c09328abdfa59bc3981cd0` run only when the recovery hash is `65a7312fe5a7829d7cd5850bc71bc3d29e57f40a991ba070c6523044b00518e3`, the generated-config hash is `afe2b4621b71c8a4a5bef19245084dfc3975ab6b0ee0f0d5af32ddf074e9b21f`, and exact run/project/account/Worker identity all match the tracked manifest. After that immutable predecessor is accepted, retry state is chained with an HMAC that cannot be reproduced from tracked constants alone. No other predecessor is accepted. `baseline-adoption-receipt.json` separately preserves the HMAC-authenticated predecessor proof used for migrated reuse; `cleanup-receipt.json` records each authenticated current-run cleanup, including pristine cleanup, without overwriting baseline provenance. Both validate run/ledger state, Ethereal non-persistence, execution budgets, Auth/data/Storage/Worker/widget final state, migration count, and catalogue counts. A later migrated-baseline execution requires the preserved baseline-adoption receipt and skips link, dry-run, push, migration replay, and catalogue reseeding.
