<script lang="ts">
  ...
  let offerTurnstileElement = $state<HTMLDivElement>();
  ...
  $effect(() => {
    ...
    const element = offerTurnstileElement;
    if (data.demoMode || !sitekey || !offerOpen || !element) return;
    ...
    <div class="offer-panel" role="dialog" aria-modal="true" aria-labelledby="offer-title" tabindex="-1" onkeydown={handleOfferKeydown}>
      ...
      {:else}
        <form method="POST" action="?/submitOffer">
          ...
          {#if data.turnstileSiteKey && !data.demoMode}<div class="cf-turnstile" bind:this={offerTurnstileElement} data-sitekey={data.turnstileSiteKey} data-action="offer_submit"></div>{/if}
