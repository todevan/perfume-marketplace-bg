<script lang="ts">
  import { AlertTriangle, Check, CheckCheck, MessageCircle, PackageCheck, Star, X } from '@lucide/svelte';
  import type { ActionData, PageData } from './$types';

  let { data, form }: { data: PageData; form: ActionData } = $props();
  const labels = { pending_confirmation: 'Чака двойно потвърждение', completed: 'Приключена', disputed: 'В спор', cancelled: 'Отказана' } as const;
...
<section class="deals-page"><div class="container">
  <header><span class="eyebrow">Двойно потвърждение</span><h1>Сделки</h1><p>Плащането и доставката остават извън платформата. Потвърди само след реално приключване.</p></header>
  ...
  <div class="deal-list">
    {#each data.deals as deal}
      <article class:highlighted={data.highlight === deal.id} class="deal-card surface">
        ...
        <div class="confirmations"><div class:done={deal.confirmedBy.includes(deal.partyA.id)}><Check size={16} /> {deal.partyA.username}</div><span></span><div class:done={deal.confirmedBy.includes(deal.partyB.id)}><Check size={16} /> {deal.partyB.username}</div></div>

        {#if deal.status === 'pending_confirmation'}
          <div class="deal-actions">
            <a href="/messages"><MessageCircle size={16} /> Към чата</a>
            ...
            {#if !deal.confirmedBy.includes(data.viewerId)}<form method="POST" action="?/confirm"><input type="hidden" name="dealId" value={deal.id} /><button class="confirm" type="submit"><CheckCheck size={17} /> Сделката приключи</button></form>{/if}
          </div>
        ...
        {:else}<p class="closed-note">{deal.cancellationReason ?? 'Тази сделка не приема нови потвърждения.'}</p>{/if}
      </article>
    {/each}
  </div>
