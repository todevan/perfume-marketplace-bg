...
<section class="compare section"><div class="container"><div class="section-heading"><div><span class="eyebrow">Сравни наличните предложения</span><h2>{data.listings.total} активни флакона.</h2></div><p>Цената и остатъкът принадлежат на конкретната обява. Плащането и доставката остават извън платформата.</p></div>{#if offers.length}<div class="offer-grid">{#each offers as offer}<ListingCard listing={offer} />{/each}</div>{:else}<div class="empty-state"><div><Bell size={34} /><h2>Няма активен флакон.</h2><p class="muted">Запази търсенето, за да получиш известие при нова обява.</p></div></div>{/if}</div></section>
...
<style>
  .perfume-hero { padding-block: 55px 80px; border-bottom: 1px solid var(--line); background: linear-gradient(150deg, rgb(255 253 249 / 35%), rgb(243 223 191 / 48%)); }
  .perfume-grid { display: grid; align-items: center; grid-template-columns: minmax(360px, .86fr) minmax(0, 1.14fr); gap: clamp(45px, 8vw, 120px); }
  .visual-wrap { overflow: hidden; border-radius: 45% 45% 18px 18px; box-shadow: var(--shadow-lg); }
  :global(.visual-wrap .visual) { min-height: 600px; }
  .brand { margin: 0; color: var(--ink-soft); font-size: 1.1rem; }
  h1 { margin: 8px 0 23px; }
  ...
  .facts-grid strong { font-size: 1.3rem; font-style: italic; }
  .facts-grid p { margin: 0; }
  .facts-grid .notice { border: 0; background: rgb(243 223 191 / 10%); }
  ...
  @media (max-width: 560px) { .visual-wrap { border-radius: var(--radius-md); } :global(.visual-wrap .visual) { min-height: 430px; } .offer-grid, .facts-grid { grid-template-columns: 1fr; } }
</style>
