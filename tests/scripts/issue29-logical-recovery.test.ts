...
test('rejects TLS downgrade, mismatched database host, and host-only certificate paths at the connection boundary', () => { const invalidRootCert: any = '/private/root.crt', options = { scope, connection: { host: 'db.abcdefghijklmnopqrst.supabase.co', port: 5432, database: 'postgres', user: 'postgres', password: 'private-test', sslmode: 'verify-full' as const }, toolchain: { mode: 'container' as const } }; expect(() => createPostgresToolchain({ ...options, connection: { ...options.connection, sslmode: 'disable' } })).toThrow('DATABASE_TARGET_MISMATCH'); expect(() => createPostgresToolchain({ ...options, connection: { ...options.connection, host: 'db.bcdefghijklmnopqrstu.supabase.co' } })).toThrow('DATABASE_TARGET_MISMATCH'); expect(() => createPostgresToolchain({ ...options, connection: { ...options.connection, sslRootCert: invalidRootCert } })).toThrow('SYSTEM_CA_ROOTS_REQUIRED'); });
test('uses only the fingerprint-guarded Supabase CA for exact session-pooler TLS and mounts it read-only', async () => { const directory = await mkdtemp(join(tmpdir(), 'issue29-pooler-tool-test-')), priorPath = process.env.PATH; try {
    const docker = `#!/usr/bin/python3\nimport os,sys,json\nprint(json.dumps({'ssl':os.environ.get('PGSSLROOTCERT'),'argv':sys.argv[1:]}))\n`;
    await writeFile(join(directory, 'docker'), docker, { mode: 0o700 }); process.env.PATH = `${directory}:${priorPath}`;
    const tools = createPostgresToolchain({ scope, connection: { host: 'aws-0-eu-central-1.pooler.supabase.com', port: 5432, database: 'postgres', user: `postgres.${scope.projectRef}`, password: 'private-test', sslmode: 'verify-full', sslRootCert: 'supabase-prod-2021' }, toolchain: { mode: 'container' } });
    const result = JSON.parse((await tools.run('psql', ['--capture'])).toString());
    expect(result.ssl).toBe('/issue29/supabase-prod-ca-2021.crt'); expect(result.argv).toContain('--mount'); expect(result.argv.some((value:string)=>value.includes('dst=/issue29/supabase-prod-ca-2021.crt,readonly'))).toBe(true); expect(result.argv).toContain('--capture');
    expect(() => createPostgresToolchain({ scope, connection: { host: 'aws-0-eu-central-1.pooler.supabase.com', port: 5432, database: 'postgres', user: `postgres.${scope.projectRef}`, password: 'private-test', sslmode: 'verify-full', sslRootCert: 'system' }, toolchain: { mode: 'container' } })).toThrow('DATABASE_TARGET_MISMATCH');
} finally { process.env.PATH = priorPath; await rm(directory, { recursive: true, force: true }); } });
