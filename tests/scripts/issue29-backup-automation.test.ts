...
function authorization() {
    ...
    manifest.monitoring.configSha256='e'.repeat(64);
...
it('keeps ambiguous independent heartbeat pending and retries only readback, never submission', async () => {
    ...
    try {
        ...
        const heartbeatConfig = { origin: 'https://issue29-monitor.owner.workers.dev', writeToken: 'w'.repeat(43), readToken: 'r'.repeat(43), environmentAlias: 'synthetic-recovery', candidateSha: context.sha, configSha256: 'e'.repeat(64) };
        const artifactBytes=await readFile(join(f.paths.directory,'artifact-readback.json'));
        const artifactSha256=createHash('sha256').update(artifactBytes).digest('hex');
        let posts = 0;
        const fetchImpl = async (_url: string | URL | Request, init?: RequestInit) => { if (init?.method === 'POST') {
            posts++;
            throw new Error('secret provider response');
        } return Response.json({schemaVersion:1,environment:'synthetic-recovery',release:context.sha,backupRelease:context.sha,configSha256:'e'.repeat(64),checkpointAt:now,descriptorSha256:f.created.descriptorSha256,artifactSha256,integrityFailureEvidenceSha256:null}); };
...
it('publishes definite local ciphertext corruption once without refreshing a checkpoint or replacing another pending operation', async () => {
 const f=await publicationFixture();try{
  ...
  const heartbeatConfig={origin:'https://issue29-monitor.owner.workers.dev',writeToken:'w'.repeat(43),readToken:'r'.repeat(43),environmentAlias:'synthetic-recovery',candidateSha:context.sha,configSha256:'e'.repeat(64)};
  let posts=0;let observed='';const fetchImpl:typeof fetch=async(url,init)=>{if(init?.method==='POST'){posts++;const m=await readPrivateManifest(f.paths.manifestPath,{repositoryRoot:process.cwd(),now});expect(m.pending).toMatchObject({step:'backup-heartbeat',resourceId:'failure'});expect(JSON.parse(String(init.body))).not.toHaveProperty('checkpointAt');observed=JSON.parse(String(init.body)).evidenceSha256;return new Response(null,{status:204});}return Response.json({schemaVersion:1,environment:'synthetic-recovery',release:context.sha,backupRelease:context.sha,configSha256:'e'.repeat(64),checkpointAt:null,descriptorSha256:null,artifactSha256:null,integrityFailureEvidenceSha256:observed});};
