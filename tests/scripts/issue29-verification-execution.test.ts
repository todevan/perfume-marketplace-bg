...
import {monitorWebhookSigningSecretSha256} from '../fixtures/issue29-monitor';
...
async function cleanupFixture(){
 ...
 const origin=`https://issue29-${m.runId}.owner.workers.dev`,config={workerAlias:'issue29-monitor',origin:'https://issue29-monitor.owner.workers.dev',targetOrigin:origin,runtimeEnvironment:'development',environmentAlias:m.fixture.alias,candidateSha:m.candidate.sha,runId:m.runId,webhookSigningSecretSha256:monitorWebhookSigningSecretSha256,evidenceReadToken:'e'.repeat(43),watchdogToken:'w'.repeat(43),backupCheckpointToken:'b'.repeat(43),maintenanceToken:'m'.repeat(43)},configuration={...config,destinationAlias:'owner-primary',configSha256:'e'.repeat(64),signals:MONITOR_SIGNALS,status:'verified'};m.monitoring.configSha256=configuration.configSha256;m.monitoring.workerAlias=config.workerAlias;
 const monitoring={schemaVersion:1,operation:'monitoring-proof',action:'source-green',actionId:'final-source-green',monitor:config,binding:{providerToken:'private-provider-token',source:{apiUrl:m.source!.url,serviceKey:'private-service-role-key'},deployment:{accountId:'c'.repeat(32),workerName:`issue29-${m.runId}`,versionId:m.candidate.deploymentId,origin,readToken:'private-cloudflare-token'}}};
 const settings={schemaVersion:1,operation:'cleanup',action:'finalize',providerToken:'private-read-token',capabilityId:m.capabilityIds.cleanup,sourceMonitoringSettingsPath:join(directory,'monitoring.json'),sourceWorker:{settings:{accountId:'c'.repeat(32),subdomain:'owner',purpose:'source',publishableKey:'sb_publishable_fixture',turnstileSiteKey:'0x4fixture',operations:{migrationSha256:'a'.repeat(64),schemaSha256:'b'.repeat(64),sentinelSha256:'c'.repeat(64),canaryExpectedUtc:'03:17'}},privateDirectory:directory,readToken:'private-cloudflare-token'},workers:[],monitorWorkers:[]};
 m.cleanup.resources.push({provider:'cloudflare-monitor',id:`worker:${config.workerAlias}`,runId:m.runId,createdAt:now,evidenceSha256:'e'.repeat(64),disposition:'persistent',absentAt:null});
 ...
 const state=()=>({lastCompletedMonitorCycleAt:now,signals:MONITOR_SIGNALS.map(signal=>({signal,ok:true}))});
 let reads=0;const dependencies={supabaseFactory:()=>({readOwnedDisposableAbsent:async({resourceId}:any)=>{reads++;return{absent:true,resourceId,evidenceSha256:'d'.repeat(64)};}}),sourceBinding:async()=>{reads++;return{sourceRef:m.source!.ref};},workerBinding:async()=>{reads++;return{workerName:`issue29-${m.runId}`};},monitorFactory:()=>({verifyConfiguration:async()=>{reads++;return configuration;},readSourceGreenState:async()=>{reads++;return state();}})};
...
it('closes the first premerge rehearsal from only its current verified owner copy, while leaving backup freshness deferred',async()=>{
 const f=await cleanupFixture();try{await installPremergeOwnerCopy(f);const prior=f.dependencies.monitorFactory;f.dependencies.monitorFactory=()=>({...prior(),readSourceGreenState:async()=>({lastCompletedMonitorCycleAt:now,signals:MONITOR_SIGNALS.map(signal=>({signal,ok:signal!=='backup_freshness'}))})});const result=await executeFinalizeCleanup(f,f.dependencies as any);const proof=JSON.parse((await readFile(join(f.directory,result.evidenceSha256+'.json'))).toString());expect(proof.deferredBackupFreshness).toBe(true);expect(proof.persistent.rules).toHaveLength(9);}finally{await rm(f.directory,{recursive:true,force:true});}
});
it.each(['stale-owner-copy','mismatched-owner-copy','github-artifact-present'])('rejects %s instead of deferring backup freshness',async kind=>{
 const f=await cleanupFixture();try{await installPremergeOwnerCopy(f,{stale:kind==='stale-owner-copy',mismatch:kind==='mismatched-owner-copy',githubArtifact:kind==='github-artifact-present'});const prior=f.dependencies.monitorFactory;f.dependencies.monitorFactory=()=>({...prior(),readSourceGreenState:async()=>({lastCompletedMonitorCycleAt:now,signals:MONITOR_SIGNALS.map(signal=>({signal,ok:signal!=='backup_freshness'}))})});await expect(executeFinalizeCleanup(f,f.dependencies as any)).rejects.toThrow(['stale-owner-copy','mismatched-owner-copy'].includes(kind)?'PREMERGE_OWNER_COPY_PROOF_INVALID':'PERSISTENT_MONITOR_NOT_GREEN');}finally{await rm(f.directory,{recursive:true,force:true});}
});
it.each(['pending','still-present','persistent-deleted','maintenance-open','unmapped-resource','not-actually-absent','monitor-firing'])('refuses %s before terminal advancement',async kind=>{
 ...
 if(kind==='monitor-firing'){const prior=f.dependencies.monitorFactory;f.dependencies.monitorFactory=()=>({...prior(),readSourceGreenState:async()=>({lastCompletedMonitorCycleAt:now,signals:MONITOR_SIGNALS.map(signal=>({signal,ok:false}))})});}
