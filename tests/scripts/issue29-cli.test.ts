...
describe('Issue 29 verification entry point', () => {
  it('identifies available local verification without claiming hosted readiness', () => {
    ...
    expect(result.stdout).toContain('Hosted mutations are unavailable');
  });
  it.each(['restore', 'backup-set', 'monitoring-proof', 'incident-drill', 'cleanup'])('fails closed for unwired hosted %s', (command) => {
