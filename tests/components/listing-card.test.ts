...
import { normalizeListing, profileKindLabel, type LegacyListing } from '../../src/lib/components/listing/card-view-model';
...
describe('ListingCard', () => {
  it('renders composition-three listing facts and canonical actions', () => {
    ...
    expect(screen.getByText('Eau de Parfum (EDP)')).toBeTruthy();
  ...
  it('normalizes legacy listing data into a reusable card view model', () => {
    const legacyListing: LegacyListing = {
      id: 'legacy-1',
      slug: 'legacy-slug',
      brand: 'Dior',
      perfume: 'Sauvage',
    ...
    const normalized = normalizeListing(legacyListing);

    expect(normalized.brandName).toBe('Dior');
    expect(normalized.fragranceName).toBe('Sauvage');
    expect(normalized.price?.amountMinor).toBe(7600);
    expect(profileKindLabel(normalized)).toBe('Частно лице');
  });
