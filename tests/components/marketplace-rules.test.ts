...
describe('Marketplace Rules deal lifecycle copy', () => {
	it('explains seller completion, participant cancellation, and review eligibility', () => {
		...
		expect(lifecycle?.textContent).toContain(
			'Преди приключването всеки от участниците може да я откаже, като посочи причина.'
		);
		expect(lifecycle?.textContent).toContain(
			'Само приключените от продавача сделки отключват възможност за отзив; отменените сделки не я отключват.'
		);
	});
});
